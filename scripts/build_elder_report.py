...
"누가 공정한지 <b>AI 심판(judge)</b>에게 직접 평가시키면 더 낫지 않을까?"를 시험한다. 결과는 정반대였다 —
심판은 <b>첫인상</b>(첫 발언)만 보고 채점하는데, 경쟁 상황이라 다들 욕심꾸러기처럼 들려서 <b>모두에게 박한 점수</b>를
줬다. 게다가 그 점수가 <b>지울 수 없는 장부</b>에 영구 기록되는 바람에, 나중에 공정하게 행동한 참가자까지 쫓겨났다.
교훈: <b>"말"보다 "행동"을 봐야 하고, 이른 판단을 영구 기록으로 굳히면 위험하다.</b> (그냥 행동만 보는 단순한
방식 <b>rule</b>이 가장 좋았다.)</div>"""
