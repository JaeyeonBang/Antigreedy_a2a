...
## cap-null 대조군 결과 (리뷰 #1·#7① — 완료)

`antigreedy/governance/nullcap.py`(더미 캡 `cap=k·remaining`, 사회 로직 없음) vs 사회 정책을
resource-task(4 에이전트, A=hog)에서 비교. `tests/test_cap_null_ablation.py`.

| 정책 | top_share | Jain | **완료율(welfare)** |
|---|---|---|---|
| 무규제 | 0.60 | 0.60 | 0.25 |
| 더미캡 k=0.22 | 0.30 | 0.97 | 0.50 |
| 정량 quota | 0.30 | 0.98 | 0.50 |
| **사회: 평판·배제** | 0.28 | 0.98 | **0.75** |
| **사회: 보편화** | 0.27 | 0.99 | **0.75** |
| 사회: 조건부 | 0.60 | 0.60 | 0.25 (효과 0) |

**결론(정직)**: ① **공정성(top_share/Jain)에서는 더미 캡 = 사회 정책** — 리뷰가 옳다, 공정성
지표만으론 사회 메커니즘을 credit 못 한다. ② **그러나 welfare(완료율)에서는 적응형 사회 정책
(0.75)이 어떤 정적 캡(0.50)보다 우월** — 적응성(풀 보존)이 실제로 더 많은 에이전트를 완주시킨다.
**즉 사회 메커니즘은 '캡 산술 이상'을 하지만, 그 차이는 오직 welfare 지표에서만 드러난다**(리뷰의
"welfare를 앞세워라"가 핵심임을 입증). ③ 조건부 협력은 first-mover hog에 무력(정직한 negative).
