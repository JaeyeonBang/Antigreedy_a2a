...
이어 우리는 문헌이 제안한 *더 정교한* 출력측 기제 셋 — 평판 망각(Beta+λ)·Elder LLM-판관 원장·진짜 이차투표(QV) — 을 같은 통제 설계로 검정했다(§6). 세 실험 모두 같은 결론으로 수렴한다: **단순한 기반이 정교한 신호를 이긴다.** 현행 누적 평판은 회복률 0의 *영구 카스트*를 만들고(망각만이 회복을 살림), LLM 판관을 섞으면 불변 원장이 이른 오판을 동결해 순수 행동 평판보다 *나빠지며*, 평판가중 QV는 무가중 QV에 이득이 없다. 단 진짜 QV는 이 프로그램에서 유일하게 독점을 줄이며 후생까지 올린 *양면 개선*으로, 그 비결은 사회적 정교함이 아니라 **고정 예산이라는 단순한 구조 제약**이었다.
...
§5가 *입력 프레이밍*의 허상을 벗겼다면, 이 절은 향후 과제로 미뤘던 **출력측 거버넌스 기제 자체를 더 정교하게** 만들어 같은 통제 검정에 건다. 문헌이 제안한 세 레버 — (B/D) **평판 망각**(Beta+λ), (D) **Elder LLM-판관 원장**, (C) **진짜 이차투표(QV)** — 를 구현하고, V6와 *동일한 방법론*(공통 기준점·bootstrap·순열검정·Holm·원자료 0불일치 재계산)으로 측정했다. 모든 수치는 **glm-4.7-flash**(reasoning off)에서 나왔고, 세 실험의 전체 결과·수식·한계는 각 리포트에 있다([평판 망각](caste_report.html) · [Elder 원장](elder_report.html) · [진짜 QV](qv_report.html)). 세 실험은 §5와 같은 결론으로 수렴한다: **단순한 기반이 정교한 신호를 이긴다.**

### 6.1 평판 망각(Beta+λ): 현행 평판은 *영구 카스트*를 만든다 (N=30)

§4의 평판 공식 `rep = clip(1 − overage/fair, 0.1, 1)`은 *누적 점유*만 본다 — 한 번 과점해 점유가 높아지면 이후 공정해져도 회복이 안 된다(자기실현 카스트). 이를 검정하려 평판을 표준 **Beta 평판**(Jøsang 2001)으로 일반화하고 **지수 망각 λ**를 끼웠다: 라운드별 행동을 의사관측치로 쌓되 `λ^Δ`로 시간 감쇠시킨다(λ=1 영구 누적, λ<1 최근 가중). 새 종속변수는 **회복률**(저평판으로 떨어진 에이전트가 임계 위로 돌아오는 속도; 0=카스트 고착).
...
**현행 선형 평판의 회복률은 정확히 0 — 한 번 찍히면 영원히 배제되는 카스트다.** 어떤 Beta 망각이든 회복을 되살리며(λ=0.7 vs 선형 `p_holm=.0007` 유의), 회복률은 λ가 작을수록 단조 증가한다. 단 Beta 내부의 미세 λ-기울기(λ0.7 vs λ1.0)는 방향은 맞으나 N=30에서 무유의(`p_holm=.50`) — 진짜 절벽은 *선형 누적 ↔ Beta* 경계에 있다. 트레이드오프도 드러난다: 엄격한 선형 배제는 독점을 가장 낮추지만(0.350) 회복을 0으로 죽이고, 망각은 회복을 살리되 개혁한 에이전트를 재참여시키느라 독점이 다소 오른다. **공정성의 극단과 회복가능성은 같은 축의 양 끝이다.**
...
**판관은 앵커를 넘어 능동적 해악이었다.** ledger_elder는 rule·numbers와 *유의하게 달랐지만*(모든 대조 `p_holm≤.0028`) **더 나쁜 쪽**으로 — 독점은 가장 높고(0.757) 후생은 무너졌다(0.33). 순수 행동 평판(rule, α=1)이 최선이다(후생 0.95). 실 LLM 진단 한 번으로 기제를 확인했다: 판관은 라운드 0에 채점하는데, 경쟁 페르소나 아래선 *모든* 에이전트의 첫 근거가 자기중심적으로 들려 전원에게 가혹한 점수(0.2~0.3)를 준다. **불변 원장이 그 이른 오판을 동결**해, 이후 공정하게 행동한 에이전트(rule 평판 0.60)까지 `elder ≈ 0.45`로 끌려 배제된다. 이는 §6.1 카스트의 *자매 결과*다 — 거기선 행동 누적이, 여기선 LLM 판단 오류가 불변성에 동결돼 카스트를 만든다. 교훈은 **"무엇을 말했나"보다 "무엇을 했나"** 가, 그리고 *이른·불변 판단이 위험*하다는 것이다.
...
**진짜 QV는 독점을 거의 절반으로 줄이면서 후생까지 올린, 이 프로그램에서 유일한 *양면 개선*이다**(독점 0.475→0.26, 후생 0.78→0.93). §5·§6.1·§6.2의 거버넌스가 모두 공정성↔후생을 맞바꾼 데 반해, QV만 둘 다 개선했다 — 2차 비용+고정 예산이 독점 시도를 *스스로 손해*로 만들기 때문이다. CI는 매우 좁아 효과가 시드마다 일관되며(raw `p=.009/.013`), 다만 6개 대조 Holm 보정에서 `.054/.065`로 *간발 미달*이다(none의 큰 분산 + 다중비교 페널티). 유의선을 넘기려 사후에 N을 늘리지 않고(goalpost 회피) 계획된 N=20을 그대로 보고한다. **평판가중(qv_rep)은 무가중(qv_flat) 대비 측정 가능한 이득이 없었다** — 예산이 먼저 binding이라 `÷rep`가 캡을 거의 안 바꾼다. Sybil(신원 분할로 2차 비용을 1/k로 깎는 공격)의 취약점과 방어(신원-귀속 합산 예산 → 분할 이득 0)는 결정론적 단위 회계로 보였다.

### 6.4 종합 — 단순한 기반이 정교한 신호를 이긴다

세 확장 실험은 §5와 *같은* 방향을 가리킨다. **복잡한 신호를 더해도 단순한 기반을 못 이긴다:** (6.1) Beta의 미세 λ-기울기는 무유의 — 진짜 차이는 "선형 ↔ Beta" 경계 하나뿐, (6.2) LLM 판관 혼합은 순수 행동 평판보다 *나빴고*, (6.3) 평판가중 QV는 무가중 QV에 이득이 없었다. §5.3의 "평판 피드백 = 앵커링"과 합치면 하나의 일관된 처방이 된다: **정교한 사회적/판단적 장치를 덧대기 전에, 그것이 단순 기반(행동 평판·고정 예산)을 *유의하게* 이기는지부터 통제 검정하라.** 한편 §6.1·§6.2는 새로운 위험을 더한다 — *불변성*은 (행동이든 LLM 판단이든) 오류를 동결해 카스트를 만든다. 그리고 §6.3은 드물게 통하는 기제(진짜 QV)가 *예산 제약*이라는 단순한 구조적 장치에서 나옴을 보인다.
...
LLM-에이전트 탐욕 거버넌스에 대한 통제·앵커대조·다중보정 실험은 **대부분의 후보 효과가 측정 착시**임을 보였다: 9개 비교 중 상위-정체성의 독점 감소와 출력 "사회" 정책의 후생 *비용*만 통제 검정을 통과했다. 평판 되먹임의 외견상 효과는 기준점 표류와 숫자 앵커링으로 설명되고, 내용 없는 배너가 후생 1등을 했으며, 정체성 프레이밍의 *해악*은 군집 내용이 아니라 분열 배너 자체에서 온다. 더 정교한 출력측 기제 셋(§6)도 같은 검정에서 같은 교훈을 냈다: **복잡한 신호를 더해도 단순한 기반을 못 이긴다** — Beta 망각의 미세 λ-기울기·LLM 판관 혼합·평판가중 QV 모두 단순 기반 대비 이득이 없거나 더 나빴고, 유일한 양면 개선(진짜 QV)은 사회적 정교함이 아니라 *고정 예산*이라는 구조 제약에서 나왔다. 동시에 §6은 새 위험을 더한다 — *불변성*(누적 평판이든 불변 판관 원장이든)은 오류를 동결해 회복 불가능한 *카스트*를 만든다. 기여는 재사용 가능한 A2A 거버넌스 토대, 앵커링-대조 방법론, 통제 검정을 통과한 효과와 통과하지 못한 효과의 명확한 구분, 그리고 메커니즘 확장에서 일반화된 "단순 기반 우선" 처방이다. 향후 과제: (1) 상위 정체성에 앵커 대조군 추가; (2) 출력 캡이 후생을 돕는/해치는 지점을 지도화하는 풀/에이전트 스윕; (3) 살아남은 효과(특히 진짜 QV)의 멀티모델 재현과 N≥100(QV의 Holm-간발 확정 포함); (4) 카스트를 푸는 *가변·감쇠 원장*과 다대상 투표 QV; (5) 간접 상호성 기제가 실제로 작동할 장기·반복 상호작용 시나리오.
