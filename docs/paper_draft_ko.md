...
### 5.6 내용과 무관한 *해악*: 창발 정체성의 위약-군집 대조 (후속·다른 모델)

본 절은 V6(GLM-4.6)와 **별도의 후속 실험**이며 모델이 다르다(**glm-4.7-flash**, reasoning off, N=30, 에이전트 3·6). 따라서 §5.1–5.5의 수치와 직접 비교하지 않고, §5.4의 *자매* 결과로 읽는다.

§5.5에서 부과형 상위 정체성("한 팀")의 독점 감소가 유일하게 견고했다. 그렇다면 정체성을 *부과*하는 대신 **관측된 행동에서 군집을 창발**시키면(누가 누구와 비슷하게 점유했나) 더 정직한 정체성이 될까? `emergent_identity` 셰이퍼(행동 점유율로 에이전트를 군집화해 "관측된 그룹"을 명시 + 동일한 상위 목표 부과)로 검정한 결과는 **역효과**였다: 창발 정체성은 독점을 줄이기는커녕 무내용 배너(`neutral_filler`)보다 **유의하게 높였다**(top-share, p_holm=.031[n=3]·.018[n=6]). 설계가 경고한 *카스트화*("탐욕 부류"로 명시 → 자기범주화로 더 독식)가 데이터로 나타났다.

핵심은 **위약-군집 대조**다. `placebo_cluster`는 emergent와 배너 문구·군집 크기·상위 목표가 모두 같되 **멤버십만 무작위**다(거짓 군집). 해악이 "군집이 실제 탐욕을 짚어서"라면 위약은 무해해야 한다. 그러나 **emergent ≈ placebo로 구별되지 않았다**(p_holm=1.0, 에이전트 3·6 양쪽). 즉 해악은 군집의 *내용*이 아니라 **분열적 "관측된 그룹" 배너를 주입하는 행위 자체**에서 온다 — 진실이든 날조든. 에이전트 6에선 위약조차 무내용 배너보다 독점을 유의하게 올렸다(p_holm=.0018) — 분열 프레임 자체가 해롭다는 직접 증거다.

이것은 §5.4의 **거울상**이다. §5.4가 *중립* 배너의 **이득**이 사회적 설계와 무관함을 보였다면, §5.6은 *분열* 배너의 **해악**이 군집 내용과 무관함을 보인다. 둘을 합치면: 측정된 입력-프레이밍 효과는 설계된 사회적 기제가 아니라 **프레임의 성격(중립 대 분열)이라는 거친 축**을 따라간다. (단, 검정력 한계상 emergent≈placebo는 "동일의 증명"이 아니라 "행동-도출이 해악을 키운다는 증거의 부재"이며, 다른 모델이라 일반성은 미주장. 상세 = `docs/verify_phase_d_placebo.md`.)
